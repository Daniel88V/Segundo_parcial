...
class Participantes(Candidatas):
    ...
    def registrar_puntajes(self, puntajes):
        ...
        if not all(criterios in puntajes for criterio in criterios):
...
class Concurso:
    ...
    def calificacion(self, participante, puntaje):
        concursante = self._buscar_participante(participante)
        if concursante:
            participante.calificacion(puntaje)
            return True
        return False
    def _buscar_participante(self, participante):
        for participante in self._participantes:
            if participante.codigo.lower() == participante.codigo.lower():
                return participante
        return None
